...
import { pow, div, add, mix, mod, round, abs, color, time, positionLocal, sin, vec3, Fn, uv, uniform, screenSize, Loop, float, fract, step } from 'three/tsl';
...
export function FractalLikeV1(props: any) {
    ...
    const main = Fn(() => {
        ...
        // })
        // const color = cosinePalette(abs(uv0.x).add(abs(uv0.y)).mul(_time), a, b, c, d);
        ...
        uvR.x = step(0.5, mod(uvR.x.mul(round(domainRepetitions / 2.0)), 1.0))

        // Add some vertical movement based on the value of uvR.x (either 0, or 1)
        const _time1 = time.mul(0.01)
        // choose regular time for a column or backwards time for a column
        _uv.y.addAssign(mix(_time1, _time1.negate(), uvR.x))

        // warp the space AFTER we have the column effect
