...
const experimentDifferentDomainsStep1 = Fn(() => {
  ...
  Loop({ start: 0, end: MAX_ITERATIONS }, ({ i }) => {
    ...
    // Create an interesting intersection between two shapes and two different domains
    // Use the fractionated domain for the box pattern
    const box = sdBox2d(_uv.mul(2.0)).toVar()

    // Use the original domain for the sphere pattern
    const circle = sdSphere(uv0).toVar()

    // Apply a smooth max function to the two shapes - this will create an interesting intersection
    const pattern = smax(box, circle, 0.1).toVar()

    // Apply a smooth min function to the two shapes - this will create a union
    // pattern.assign(smin(box, circle, 0.3))
...
const sketch = new WebGPUSketch(canvas, experimentDifferentDomainsStep1())
