...
import { useControls, folder, button, LevaInputs } from "leva";
import { cosinePalette } from "./tsl/utils/color/cosine_palette";

// Fixed-weight fonts: load all available weights
...
// Variable fonts: load single weight (browser will interpolate others)
...
export function LineCircle(props: any) {
  ...
  const uTextColor = useMemo(
    () => uniform(new THREE.Vector3(1.0, 1.0, 1.0)),
    [],
  );
  ...
  const uTextX = useMemo(() => uniform(0.3), []);
  ...
  const [controls, set] = useControls("Line Circle", () => ({
    ...
    "Mask Settings": folder({
      triEnabled: {
        value: true,
        label: "Triangle Enabled",
        onChange: (v: boolean) => {
          uTriEnabled.value = v ? 1.0 : 0.0;
        },
      },
      triRotation: {
        value: 0,
        ...
        step: 1,
        onChange: (v: number) => {
          uTriRotation.value = (v * Math.PI) / 180;
        },
      },
      triSize: {
        value: 1.0,
      ...
      triWidth: {
        value: 45,
        label: "Triangle Base Width",
        min: 5,
        max: 89,
        step: 1,
        onChange: (v: number) => {
          uTriWidth.value = (v * Math.PI) / 180;
        },
      },
      centerCircleEnabled: {
        value: true,
        label: "Center Circle",
        onChange: (v: boolean) => {
          uCenterCircleEnabled.value = v ? 1.0 : 0.0;
        },
      },
      centerCircleRadius: {
        value: 0.04,
    ...
    "Text Settings": folder({
      text: { value: "2001", label: "Text", type: LevaInputs.STRING },
      fontFamily: {
        value: "Montserrat",
      ...
      textX: { value: 0.3, min: 0.0, max: 1.0, step: 0.01, label: "Text X" },
      textY: { value: 0.85, min: 0.0, max: 1.0, step: 0.01, label: "Text Y" },
      useCustomTextColor: {
        value: false,
        label: "Custom Text Color",
        onChange: (v: boolean) => {
          uUseTextColor.value = v ? 1.0 : 0.0;
        },
      },
      textColor: {
        value: "#ffffff",
  ...
  const exportSettingsRef = useRef({ exportWidth: 4000, exportHeight: 4000 });
  ...
  const handleExport = useCallback(() => {
    ...
    const originalAspectUniform = uAspect.value;
    ...
    uAspect.value = originalAspectUniform;
  ...
  useControls(
    "Line Circle",
    {
      "Export Image": button(handleExport),
    },
    [handleExport],
  );
  ...
  const material = useMemo(() => {
    const main = Fn(() => {
      ...
      const circleMask = float(1).sub(
        smoothstep(aaCircle.negate(), aaCircle, circleSDF),
      );
      ...
      const lineMask = float(1).sub(
        smoothstep(lineWidth.sub(aaLine), lineWidth.add(aaLine), phase),
      );
      ...
      const centerInner = float(1).sub(
        smoothstep(aaCenter.negate(), aaCenter, centerSDF),
      );
      const centerMask = mix(
        float(1),
        float(1).sub(centerInner),
        uCenterCircleEnabled,
      );

      // Circle + lines + triangle + center circle base output
      const base = palColor.mul(
        circleMask.mul(lineMask).mul(triMask).mul(centerMask),
      );
      ...
      const textUV = vec2(textDelta.x.mul(uAspect), textDelta.y).add(
        textAnchor,
      );
      ...
      // Alpha: opaque where mask is visible, or where text is drawn
      const visibilityMask = circleMask.mul(lineMask).mul(triMask).mul(centerMask);
      const textAlpha = fillSample.add(outlineSample);
      const finalAlpha = mix(visibilityMask, float(1), textAlpha);
